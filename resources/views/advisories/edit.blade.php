...
<div class="container" style="max-width: 680px;">
    <div style="margin-bottom: 1.5rem;">
        ...
        <h1 class="page-title" style="margin-top: 8px;">Edit Advisory</h1>
        <p class="page-subtitle">{{ $advisory->crop->name }} / {{ $advisory->season }} / {{ $advisory->weather_condition }}</p>
    </div>

    <div class="card">
        <div class="card-body">
            <form action="{{ route('advisories.update', $advisory->id) }}" method="POST">
                @csrf @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select id="crop_id" name="crop_id" class="form-control" required>
                        @foreach($crops as $crop)
                            <option value="{{ $crop->id }}" {{ (old('crop_id', $advisory->crop_id)) == $crop->id ? 'selected' : '' }}>{{ $crop->name }}</option>
                        @endforeach
                    </select>
                    ...
                    <div class="form-group">
                        ...
                        <select id="season" name="season" class="form-control" required>
                            @foreach($seasons as $s)
                                <option value="{{ $s }}" {{ old('season', $advisory->season) == $s ? 'selected' : '' }}>{{ $s }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select id="weather_condition" name="weather_condition" class="form-control" required>
                            @foreach($conditions as $c)
                                <option value="{{ $c }}" {{ old('weather_condition', $advisory->weather_condition) == $c ? 'selected' : '' }}>{{ $c }}</option>
                            @endforeach
                        </select>
                ...
                <div class="form-group">
                    <label class="form-label" for="advice">Advisory Text *</label>
                    <textarea id="advice" name="advice" class="form-control" rows="5" required>{{ old('advice', $advisory->advice) }}</textarea>
                    @error('advice') <p style="color:var(--red-500);font-size:0.82rem;margin-top:4px;">{{ $message }}</p> @enderror
                </div>

                <div style="display:flex; gap:12px; margin-top:8px;">
                    <button type="submit" class="btn btn-primary">💾 Update Advisory</button>
